...
Form.Label = function FormLabel(props: FormLabelPropsI) {
  return (
    <label
      {...props}
      className="mb-2 inline-block text-lg leading-3xl text-white font-semibold capitalize"
    />
  );
};
...
Form.HelperText = function FormHelperText(props: FormHelperTextPropsI) {
  ...
  return (
    <p
      {...rest}
      className={`text-xs  text-red-800 tracking-[1.5px] text-shadow-err absolute bottom-[-10px] ${otherStyles} ${color}`}
    />
  );
};
