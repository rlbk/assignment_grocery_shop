...
const Table = ({ children }: TablePropsI) => {
  return (
    <div>
      <table className="w-full border border-solid border-primary overflow-x-auto">
        {children}
      </table>
    </div>
  );
...
Table.Th = ({ children }: TableThPropsI) => {
  return (
    <th className="text-base font-bold leading-xl min-w-[100px]  py-1 capitalize">
      {children}
    </th>
  );
};
...
Table.Col = ({ data, children }: TableTdPropsI) => {
  return (
    <td
      align="center"
      className="text-primary border-t border-solid border-primary py-3"
    >
      {data || children || "..."}
    </td>
  );
...
Table.Empty = ({ colSpan, text }: EmptyTdPropsI) => {
  return (
    <tr>
      <td
        className={`text-center p-[8rem] text-2xl bg-slate-100`}
        colSpan={colSpan}
      >
        {text || "No Data Found"}
      </td>
